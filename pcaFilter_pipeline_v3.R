# PCA filetring
# v3 : add AUC on groups identification
# 2013-07-21
############################
...
# Visualize performances according to axes:
...
# ROC curve PCA-filter Vs. Var-filter: compute the number of randmon probes removed Vs. good probes lost.
# Compare the filtering perf: S-filter quantile defined such that the same number of probes is filtered.

newScore <- pcaTraceQ(eset, pcaProbes, Dim = 2:3, Plot = FALSE)
...
# Save in synapse.
filePath <- paste0(mainPath, '/',propertyValue(e, 'name'), '_Results_Q_v3.rds')
