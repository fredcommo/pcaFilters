...
pcaTraceQ <- function(Data, PCA, Dim = 2:3, weight = 0.25, Normalize = TRUE, Plot = TRUE,...){
  require(parallel)
...
#     xTarg = xc - b*log10(((Fmax-Fb)/(yTarg - Fb))^(1/d) - 1)
    target <- .invRichardQ(pcaScore$lModel, p)
    aTarg <- 10^(target$xTarg)
    alpha = 10^(-aTarg)
    Pmax = 1 - alpha
...
.RichardQ <- function(x, y, w = 0.25, Plot = FALSE, add.points = FALSE, add.tan = FALSE,
                      add.intercept = FALSE, pcol = "royalblue1", add.line = FALSE,
